...
def load_spot_data(AreaCode):
    ...
    allSpotList=pd.read_csv('data\RecSpotTblData.csv')
    if areaCode<40:
...
def load_food_data(AreaCode):
    areaCode = int(AreaCode)
    allFoodList=pd.read_csv('data\RecFoodData.csv')
    if areaCode<40:
...
"""
def load_model():
    model_filename='tourw2v.model'
    model = Word2Vec.load('models\''+model_filename)
    return model
"""
